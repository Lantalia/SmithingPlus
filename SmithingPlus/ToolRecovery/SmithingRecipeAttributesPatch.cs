...
public class SmithingRecipeAttributesPatch
{
        ...
        ItemStack stack)
    {
        ...
        {
            __result = __result.Where(r => r.Ingredient.RecipeAttributes?[ModRecipeAttributes.NuggetRecipe]?.AsBool() != true
            ).ToList();
        }
        if (__instance is not CollectibleBehaviorAnvilWorkable)
        {
            __result = __result.Where(r => r.Ingredient.RecipeAttributes?[ModRecipeAttributes.WorkableRecipe]?.AsBool() != true
            ).ToList();
        }
    ...
    public static void PatchIfEnabled(bool condition, Harmony harmony)
    {
        ...
        foreach (var type in targets)
        {
            ...
            var postfix = new HarmonyMethod(typeof(SmithingRecipeAttributesPatch).GetMethod(nameof(GetMatchingRecipes_Postfix)));
